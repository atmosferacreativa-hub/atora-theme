<?php
/**
 * Archive Course Template Override — Atora Them v1.1.0
 *
...
if ( ! have_posts() ) {
	?>
	<div class="atora-them-archive-empty">
		<h1><?php esc_html_e( 'No hay cursos disponibles', 'atora-them' ); ?></h1>
		<p><?php esc_html_e( 'Por favor, intenta más tarde.', 'atora-them' ); ?></p>
	</div>
	<?php
	get_footer();
	return;
...
<div class="atora-them-archive-wrapper">
	<!-- Hero del archivo -->
	<div class="atora-them-archive-hero" style="background: linear-gradient(180deg, var(--as-cream) 0%, var(--as-bone) 100%); padding: 72px 0;">
		<div class="atora-them-container">
			<p class="atora-them-eyebrow">· Cursos disponibles</p>
	...
	<div class="atora-them-container" style="padding: 72px 20px;">
		<!-- Grid 3 columnas -->
		<div class="atora-them-posts-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 32px;">
			<?php while ( have_posts() ) : ?>
				<?php the_post(); ?>
				<div class="atora-them-course-card" style="background: var(--as-surface); border: 1px solid var(--as-rule); border-radius: var(--as-radius-lg); box-shadow: var(--as-shadow-sm); overflow: hidden; display: flex; flex-direction: column;">
					<!-- Featured image 4:3 -->
					<div style="position: relative; width: 100%; aspect-ratio: 4/3; overflow: hidden;">
						<?php if ( has_post_thumbnail() ) : ?>
							<?php the_post_thumbnail( 'atora-them-card', array( 'style' => 'width: 100%; height: 100%; object-fit: cover;' ) ); ?>
						<?php else : ?>
